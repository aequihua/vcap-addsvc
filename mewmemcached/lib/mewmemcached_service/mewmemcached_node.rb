...
class VCAP::Services::Mewmemcached::Node

  include VCAP::Services::Mewmemcached::Common
  include VCAP::Services::Mewmemcached
  ...
  def initialize(options)
    ...
    @mewmemcached_server_path = options[:mewmemcached_server_path]
  ...
  def shutdown
    ...
    ProvisionedService.all.each do |instance|
      stop_mewmemcached_server(instance)
    end
    true
  end
  ...
  def provision(plan, credentials = nil, db_file = nil)
    ...
    # Sleep 1 second to wait for mewmemcached instance start
    sleep 1
  ...
  def bind(instance_id, binding_options = :all, credentials = nil)
    # FIXME: Mewmemcached has no user level security, just return provisioned credentials.
    instance = nil
    if credentials
  ...
  def unbind(credentials)
    # FIXME: Mewmemcached has no user level security, so has no operation for unbinding.
    {}
  end
  ...
  def restore(instance_id, backup_dir)
    ...
    if File.exists?(dump_file)
      if File.new(dump_file).size > 0
        ...
        mewmemcached = Mewmemcached.new({:port => instance.port, :password => instance.password})
        mewmemcached.flushall
      end
    else
      raise MewmemcachedError.new(MewmemcachedError::MEWMEMCACHED_RESTORE_FILE_NOT_FOUND, dump_file)
    end
    {}
  end
  ...
  # 1. Try to use password in credentials to connect to mewmemcached instance
  ...
  def dump_instance(service_credentials, binding_credentials_list = [], dump_dir)
    ...
    # This will activate the mewmemcached instance to do snapshot in 1 second.
  ...
  def save_instance(instance)
    raise MewmemcachedError.new(MewmemcachedError::MEWMEMCACHED_SAVE_INSTANCE_FAILED, instance.inspect) unless instance.save
  end

  def destroy_instance(instance)
    raise MewmemcachedError.new(MewmemcachedError::MEWMEMCACHED_DESTORY_INSTANCE_FAILED, instance.inspect) unless instance.destroy
  end

  def get_instance(name)
    instance = ProvisionedService.get(name)
    raise MewmemcachedError.new(MewmemcachedError::MEWMEMCACHED_FIND_INSTANCE_FAILED, name) if instance.nil?
    instance
  end
  ...
  def start_instance(instance, db_file = nil)
    ...
    if pid
      ...
      $0 = "Starting Mewmemcached instance: #{instance.name}"
      close_fds
      ...
      swap_file = File.join(dir, "mewmemcached.swap")
      ...
      config_path = File.join(dir, "mewmemcached.conf")
    ...
    #  exec("#{@mewmemcached_server_path} #{config_path}")
      exec(“#{@mewmemcached_server_path}”)
    end
  rescue => e
    raise MewmemcachedError.new(MewmemcachedError::MEWMEMCACHED_START_INSTANCE_FAILED, instance.inspect)
  end
  ...
  def cleanup_instance(instance)
    ...
    raise MewmemcachedError.new(MewmemcachedError::MEWMEMCACHED_CLEANUP_INSTANCE_FAILED, err_msg.inspect) if err_msg.size > 0
  end
  ...
  def memory_for_instance(instance)
    case instance.plan
      when :free then 16
        ...
        raise MewmemcachedError.new(MewmemcachedError::MEWMEMCACHED_INVALID_PLAN, instance.plan)
    end
  end

  def stop_mewmemcached_server(instance)
    mewmemcached = Mewmemcached.new({:port => instance.port, :password => instance.password})
    begin
      mewmemcached.shutdown
    rescue => e
      if e.class == Errno::ECONNREFUSED
        # FIXME: it will raise exception even if shutdown successfully,
        # should be a mewmemcached ruby binding bug. Here just ignore it.
      elsif e.class == RuntimeError
        # It could be a disabled instance
        mewmemcached = Mewmemcached.new({:port => instance.port, :password => @disable_password})
        begin
          mewmemcached.shutdown
        rescue => e
        end
  ...
  def check_password(port, password)
    mewmemcached = Mewmemcached.new({:port => port})
    mewmemcached.auth(password)
    true
  rescue => e
    if e.message == "ERR invalid password"
      return false
    else
      raise MewmemcachedError.new(MewmemcachedError::MEWMEMCACHED_CONNECT_INSTANCE_FAILED)
    end
  ensure
    begin
      mewmemcached.quit if mewmemcached
    rescue => e
    end
  end

  def get_info(port, password)
    mewmemcached = Mewmemcached.new({:port => port, :password => password})
    mewmemcached.info
  rescue => e
    raise MewmemcachedError.new(MewmemcachedError::MEWMEMCACHED_CONNECT_INSTANCE_FAILED)
  ensure
  ...
  def get_config(port, password, key)
    mewmemcached = Mewmemcached.new({:port => port, :password => password})
    mewmemcached.config(:get, key)[key]
  rescue => e
    raise MewmemcachedError.new(MewmemcachedError::MEWMEMCACHED_CONNECT_INSTANCE_FAILED)
  ensure
    begin
  ...
  def set_config(port, password, key, value)
    mewmemcached = Mewmemcached.new({:port => port, :password => password})
    mewmemcached.config(:set, key, value)
  rescue => e
    raise MewmemcachedError.new(MewmemcachedError::MEWMEMCACHED_CONNECT_INSTANCE_FAILED)
  ensure
    begin
