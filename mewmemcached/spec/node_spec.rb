...
require "mewmemcached_service/mewmemcached_error"

module VCAP
  module Services
    module Mewmemcached
      class Node
         attr_reader :base_dir, :mewmemcached_server_path, :local_ip, :available_memory, :max_memory, :max_swap, :node_id, :config_template, :free_ports
...
describe VCAP::Services::Mewmemcached::Node do

  before :all do
    @logger = Logger.new(STDOUT, "daily")
    @logger.level = Logger::ERROR
    @local_db_file = "/tmp/mewmemcached_node_" + Time.now.to_i.to_s + ".db"
    @options = {
      :logger => @logger,
      :base_dir => "/var/vcap/services/mewmemcached/instances",
      :mewmemcached_server_path => "mewmemcached-server",
      :local_ip => "127.0.0.1",
      :available_memory => 4096,
      :max_memory => 16,
      :max_swap => 32,
      :node_id => "mewmemcached-node-1",
      :config_template => File.expand_path("../resources/mewmemcached.conf.erb", File.dirname(__FILE__)),
    ...
    EM.run do
      @node = VCAP::Services::Mewmemcached::Node.new(@options)
      EM.add_timer(0.1) {EM.stop}
    end
  end

  before :each do
    @instance          = VCAP::Services::Mewmemcached::Node::ProvisionedService.new
  ...
  describe 'Node.initialize' do
    ...
    it "should set up a mewmemcached server path" do
      @node.mewmemcached_server_path.should be @options[:mewmemcached_server_path]
    end
    ...
    it "should load the mewmemcached configuration template" do
  ...
  describe 'Node.start_provisioned_instances' do
    ...
    it "should not start a new instance if the instance is already started when start all provisioned instances" do
      ...
      instance = VCAP::Services::Mewmemcached::Node::ProvisionedService.get(@instance.name)
  ...
  describe "Node.provision" do
    ...
    it "should access the instance using the credentials returned by successful provision" do
      mewmemcached = Mewmemcached.new({:port => @credentials["port"], :password => @credentials["password"]})
      mewmemcached.get("test_key").should be_nil
    end

    it "should not allow null credentials to access the instance" do
      mewmemcached = Mewmemcached.new({:port => @credentials["port"]})
      expect {mewmemcached.get("test_key")}.should raise_error(RuntimeError)
    end

    it "should not allow wrong credentials to access the instance" do
      mewmemcached = Mewmemcached.new({:port => @credentials["port"], :password => "wrong_password"})
      expect {mewmemcached.get("test_key")}.should raise_error(RuntimeError)
    end
  ...
  describe "Node.unprovision" do
    ...
    it "should not access the instance when doing unprovision" do
      mewmemcached = Mewmemcached.new({:port => @credentials["port"], :password => @credentials["password"]})
      expect {mewmemcached.get("test_key")}.should raise_error(Errno::ECONNREFUSED)
    end
    ...
    it "should raise exception when unprovision an non-existed name" do
      expect {@node.unprovision("non-existed")}.should raise_error(VCAP::Services::Mewmemcached::MewmemcachedError)
    end
  end
  ...
  describe "Node.save_instance" do
    it "should raise exception when save instance failed" do
      ...
      expect {@node.save_instance(@instance)}.should raise_error(VCAP::Services::Mewmemcached::MewmemcachedError)
    end
  end
  ...
  describe "Node.destory_instance" do
    it "should raise exception when destroy instance failed" do
      instance = VCAP::Services::Mewmemcached::Node::ProvisionedService.new
      expect {@node.destroy_instance(instance)}.should raise_error(VCAP::Services::Mewmemcached::MewmemcachedError)
    end
  end

  describe "Node.get_instance" do
    it "should raise exception when get instance failed" do
      expect {@node.get_instance("non-existed")}.should raise_error(VCAP::Services::Mewmemcached::MewmemcachedError)
    end
  end
  ...
  describe "Node.bind" do
    ...
    it "should access mewmemcached server using the returned credential" do
      mewmemcached = Mewmemcached.new({:port => @binding_credentials["port"], :password => @binding_credentials["password"]})
      mewmemcached.get("test_key").should be_nil
    end

    it "should not allow null credentials to access the instance" do
      mewmemcached = Mewmemcached.new({:port => @binding_credentials["port"]})
      expect {mewmemcached.get("test_key")}.should raise_error(RuntimeError)
    end

    it "should not allow wrong credentials to access the instance" do
      mewmemcached = Mewmemcached.new({:port => @binding_credentials["port"], :password => "wrong_password"})
      expect {mewmemcached.get("test_key")}.should raise_error(RuntimeError)
    end
  ...
  describe "Node.memory_for_instance" do
    ...
    it "should raise exception when giving wrong plan name" do
      ...
      expect {@node.memory_for_instance(instance)}.should raise_error(VCAP::Services::Mewmemcached::MewmemcachedError)
    end
  end
  ...
  describe "Node.restore" do
    before :all do
      @restore_dir = "/tmp/restore/mewmemcached"
      FileUtils.mkdir_p(@restore_dir)
      @credentials1 = @node.provision(:free)
      @credentials2 = @node.provision(:free)
      sleep 1
      Mewmemcached.new({:port => @credentials1["port"], :password => @credentials1["password"]}).set("test_key", "test_value")
    ...
    it "should restore user data from a backup file" do
      Mewmemcached.new({:port => @credentials2["port"], :password => @credentials2["password"]}).get("test_key").should == "test_value"
    end
    ...
    it "should restore from an empty dump file" do
      restore_dir = "/tmp/restore/mewmemcached_empty"
      FileUtils.mkdir_p(restore_dir)
      FileUtils.touch(File.join(restore_dir, "dump.rdb"))
      restore_result = @node.restore(@credentials2["name"], restore_dir)
      sleep 1
      Mewmemcached.new({:port => @credentials2["port"], :password => @credentials2["password"]}).get("test_key").should be_nil
    end

    it "should raise exception if the instance is not existed" do
      expect {@node.restore("non-existed", @restore_dir)}.should raise_error(VCAP::Services::Mewmemcached::MewmemcachedError)
    end

    it "should raise exception if the dumped file is not existed" do
      expect {@node.restore(@credentials2["name"], "/tmp/restore")}.should raise_error(VCAP::Services::Mewmemcached::MewmemcachedError)
    end
  end
  ...
  describe "Node.migration" do
    before :all do
      @credentials = @node.provision(:free)
      sleep 1
      Mewmemcached.new({:port => @credentials["port"], :password => @credentials["password"]}).set("test_key", "test_value")
      @dump_dir = File.join("/tmp/migration/mewmemcached", @credentials["name"])
    ...
    it "should not access mewmemcached server after disable the instance" do
      @node.disable_instance(@credentials, @binding_credentials_list)
      sleep 1
      expect {@node.get_info(@credentials["port"], @credentials["password"])}.should raise_error(VCAP::Services::Mewmemcached::MewmemcachedError)
    end
    ...
    it "should access mewmemcached server in old node after enable the instance" do
    ...
    it "should import db file from right location after import instance" do
      ...
      Mewmemcached.new({:port => credentials_list[0]["port"], :password => credentials_list[0]["password"]}).get("test_key").should == "test_value"
      credentials_list[1].each do |key, value|
        Mewmemcached.new({:port => value["credentials"]["port"], :password => value["credentials"]["password"]}).get("test_key").should == "test_value"
      end
    end
  end
