...
module VCAP
  module Services
    module Memcached
      class Node
         attr_reader :base_dir, :memcached_server_path, :local_ip, :available_memory, :max_memory, :max_swap, :node_id, :config_template, :free_ports
...
describe VCAP::Services::Memcached::Node do
  ...
  before :all do
    ...
    @options = {
      ...
      :memcached_server_path => "memcached-server",
      :local_ip => "127.0.0.1",
  ...
  describe 'Node.initialize' do
    ...
    it "should load the memcached configuration template" do
  ...
  describe "Node.provision" do
    ...
    it "should not allow null credentials to access the instance" do
      memcached = Memcached.new({:port => @credentials["port"]})
    ...
    it "should not allow wrong credentials to access the instance" do
      memcached = Memcached.new({:port => @credentials["port"], :password => "wrong_password"})
      expect {memcached.get("test_key")}.should raise_error(RuntimeError)
    end
  ...
  describe "Node.unprovision" do
    ...
    it "should not access the instance when doing unprovision" do
      memcached = Memcached.new({:port => @credentials["port"], :password => @credentials["password"]})
      expect {memcached.get("test_key")}.should raise_error(Errno::ECONNREFUSED)
    end
  ...
  describe "Node.bind" do
    ...
    it "should access memcached server using the returned credential" do
      memcached = Memcached.new({:port => @binding_credentials["port"], :password => @binding_credentials["password"]})
      memcached.get("test_key").should be_nil
    end

    it "should not allow null credentials to access the instance" do
      memcached = Memcached.new({:port => @binding_credentials["port"]})
      expect {memcached.get("test_key")}.should raise_error(RuntimeError)
    end

    it "should not allow wrong credentials to access the instance" do
      memcached = Memcached.new({:port => @binding_credentials["port"], :password => "wrong_password"})
      expect {memcached.get("test_key")}.should raise_error(RuntimeError)
    end
  ...
  describe "Node.restore" do
    before :all do
      @restore_dir = "/tmp/restore/memcached"
      FileUtils.mkdir_p(@restore_dir)
      @credentials1 = @node.provision(:free)
      @credentials2 = @node.provision(:free)
      sleep 1
      Memcached.new({:port => @credentials1["port"], :password => @credentials1["password"]}).set("test_key", "test_value")
    ...
    it "should restore user data from a backup file" do
      Memcached.new({:port => @credentials2["port"], :password => @credentials2["password"]}).get("test_key").should == "test_value"
    end
    ...
    it "should restore from an empty dump file" do
      restore_dir = "/tmp/restore/memcached_empty"
      FileUtils.mkdir_p(restore_dir)
      FileUtils.touch(File.join(restore_dir, "dump.rdb"))
      restore_result = @node.restore(@credentials2["name"], restore_dir)
      sleep 1
      Memcached.new({:port => @credentials2["port"], :password => @credentials2["password"]}).get("test_key").should be_nil
    end

    it "should raise exception if the instance is not existed" do
      expect {@node.restore("non-existed", @restore_dir)}.should raise_error(VCAP::Services::Memcached::MemcachedError)
    end

    it "should raise exception if the dumped file is not existed" do
      expect {@node.restore(@credentials2["name"], "/tmp/restore")}.should raise_error(VCAP::Services::Memcached::MemcachedError)
    end
  end
  ...
  describe "Node.migration" do
    before :all do
      @credentials = @node.provision(:free)
      sleep 1
      Memcached.new({:port => @credentials["port"], :password => @credentials["password"]}).set("test_key", "test_value")
      @dump_dir = File.join("/tmp/migration/memcached", @credentials["name"])
    ...
    it "should not access memcached server after disable the instance" do
    ...
    it "should access memcached server in old node after enable the instance" do
    ...
    it "should import db file from right location after import instance" do
      ...
      Memcached.new({:port => credentials_list[0]["port"], :password => credentials_list[0]["password"]}).get("test_key").should == "test_value"
      credentials_list[1].each do |key, value|
        Memcached.new({:port => value["credentials"]["port"], :password => value["credentials"]["password"]}).get("test_key").should == "test_value"
      end
    end
  end
