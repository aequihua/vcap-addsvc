  ...
  module Services
    module Memcached
      class MemcachedError < VCAP::Services::Base::Error::ServiceError
        ...
        REDIS_CONNECT_INSTANCE_FAILED     = [31107, HTTP_INTERNAL, "Could not connect memcached instance"]
        REDIS_SET_INSTANCE_PASS_FAILED    = [31108, HTTP_INTERNAL, "Could not set memcached instance password"]
        REDIS_RESTORE_FILE_NOT_FOUND      = [31109, HTTP_INTERNAL, "Could not find memcached restore data file %s"]
      end
    end
  end
end
