...
class VCAP::Services::Memcached::Node
  ...
  def create_instance_cmdline(config)
  # Arturo Equihua customization, to assemble the right memcached command line
    cmdline = "#{@memcached_server_path} "
    config.each_line do  |s|
       temp = s.split
       if temp.length>1
          if temp[0].downcase=="port"·
             cmdline = cmdline + " -p"+temp[1]
         else 
         if temp[0].downcase=="daemonize"
            cmdline = cmdline + " -D"
         end
       end
      end
    end·  
    puts “ I will execute: “ + cmdline
    cmdline
  end
  ...
  def start_instance(instance, db_file = nil)
    ...
    # AE config_path = File.join(dir, "memcached.conf")
...
# AE     FileUtils.rm_f(config_path)
# AE     File.open(config_path, "w") {|f| f.write(config)}

# Arturo Equihua customization to assemble command line for memcached
      cmdline = create_instance_cmdline(config)  
      exec(cmdline)
    end
  rescue => e
